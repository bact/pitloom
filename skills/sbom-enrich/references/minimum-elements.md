...
| Component Hash Value / Algorithm | `verifiedUsing` (`{"algorithm": "sha256", "hashValue": ...}`); set on dataset files/packages, lock-file-resolved dependencies (via SHA-256 digests in supported lock files), and PyPI-resolved deps (`_extract_release_hash` fallback); **not** set on the main project package (a `loom project`-level SBOM describes source, not a built artifact) | conditional -- present on deps only when a definite version resolves; absent by design on a source-level main package (ask whether a built-artifact hash is even expected before treating this as a gap) |
