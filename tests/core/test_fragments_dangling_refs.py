...
and :func:`_warn_about_dangling_references` -- the referential-integrity check
...
merged graph. The prototypical cause is a fragment built against a base SBOM's
old ``doc_uuid`` (see
...
def test_warn_about_dangling_references_logs_one_warning_per_dangling_ref(
    ...
    stale-fragment merge is diagnosable, not a silent no-op."""
    ci = _creation_info()
    pkg = spdx3.software_Package(
        spdxId="https://spdx.org/spdxdocs/x-1#Package-1", name="pkg", creationInfo=ci
    )
