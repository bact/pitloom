...
def test_read_locked_dependencies_setting_defaults_true_when_absent() -> None:
    """Unlike ``offline``/``enrich``, this is opt-out: on by default."""
    assert _read_locked_dependencies_setting({}) is True


def test_read_locked_dependencies_setting_explicit_false() -> None:
    assert _read_locked_dependencies_setting({"locked-dependencies": False}) is False


def test_read_locked_dependencies_setting_non_bool_raises() -> None:
    with pytest.raises(ValueError, match="'locked-dependencies' must be a boolean"):
        _read_locked_dependencies_setting({"locked-dependencies": "yes"})


def test_parse_pitloom_config_locked_dependencies_default_true() -> None:
    assert parse_pitloom_config({}).locked_dependencies is True


def test_parse_pitloom_config_locked_dependencies_false() -> None:
    config = parse_pitloom_config({"tool": {"pitloom": {"locked-dependencies": False}}})
    assert config.locked_dependencies is False
