...
class ProjectFile:
    ...
    Attributes:
        ...
        content_type: A real IANA media type detected from the file's
            content/filename (``magika`` or a ``mimetypes`` fallback), or
            asserted directly by a
            ``[[tool.pitloom.file-headers.content-type-overrides]]``
            config match, independent of ``file_type`` -- see
        ...
        content_type_method: ``"magika"``, ``"mimetype_extension_guess"``,
            or ``"config_override"`` -- which tool (or config match)
