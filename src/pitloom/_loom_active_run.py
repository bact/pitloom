...
See also: :mod:`pitloom.loom`, the public facade this module is split out
of -- ``loom.py`` re-exports :class:`_ActiveRun` and stays the documented
import path (``from pitloom import loom``); nothing outside ``pitloom.loom``
...
#: Deliberately shares the "pitloom.loom" logger namespace: this module is
#: a private split-out of loom.py's internals, and its messages should read
#: as coming from the public loom SDK, not from an implementation detail
#: module callers never import directly.
...
def _get_caller_info() -> str:
    ...
    "The pitloom.loom module" spans two files: this one and the public
    facade ``loom.py`` -- its module-level convenience functions
    (``loom.set_model()`` and friends) are on the stack between the actual
    caller and here, and must be skipped just like frames from this file.
    """
    try:
        for frame_info in inspect.stack():
            if frame_info.filename not in (__file__, loom.__file__):
...
def _get_caller_script_path() -> str | None:
    ...
    Skips frames inside ``pitloom.loom`` (both this file and the public
    facade ``loom.py`` -- see :func:`_get_caller_info`) and ``contextlib``
    -- the latter appears as the immediate caller when ``loom.run`` is used
    as a function decorator, since ``contextlib.ContextDecorator.__call__``
    enters the context manager (calling ``Run.__enter__``, where this is
    invoked) *before* the wrapped function itself is called, so the wrapped
    ...
    try:
        for frame_info in inspect.stack():
            ...
            if filename in (__file__, loom.__file__, contextlib.__file__):
                continue
